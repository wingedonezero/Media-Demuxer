...
def get_base_name(path: Path) -> str:
    """Get a clean base name from the input path."""
    if path.is_dir() and path.name.lower() in ("video_ts", "bdmv"):
        return path.parent.name
    return path.stem

def detect_source_type(path: Path) -> str:
    """
    Detect whether a source is DVD or Blu-ray.

    Returns:
        'dvd' for DVD sources
        'bluray' for Blu-ray sources
    """
    if path.is_dir():
        # Check for DVD structure
        if (path / "VIDEO_TS").exists() or path.name.lower() == "video_ts":
            return "dvd"
        # Check for Blu-ray structure
        if (path / "BDMV").exists() or path.name.lower() == "bdmv":
            return "bluray"
    elif path.is_file() and path.suffix.lower() == '.iso':
        # For ISO files, we'd need to mount or inspect to determine type
        # For now, default to DVD (can be enhanced later)
        return "dvd"

    return "dvd"  # Default to DVD
...
def find_dvd_sources(path: Path) -> list[Path]:
    ...
    if path.is_file() and path.suffix.lower() == '.iso':
        ...
        if (path / "VIDEO_TS").is_dir():
            ...
            # Recursively search for ISOs and VIDEO_TS folders
